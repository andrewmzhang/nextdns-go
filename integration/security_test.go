...
func TestSecurityLifecycle(t *testing.T) {
	...
	profile, err := client.Profiles().Create(ctx, basicProfile)
	defer func() {
		// Delete test
		err := client.Profiles().Bind(profile.ID).Delete(ctx)
		require.NoError(t, err, "Error deleting profile id %s: %v", profile.ID, err)
	}()

	// Read test
	security, err := client.Security(profile.ID).Get(ctx)
	...
	err = client.Security(profile.ID).Update(ctx, security)
	require.NoError(t, err)
	security, err = client.Security(profile.ID).Get(ctx)
	require.NoError(t, err)
