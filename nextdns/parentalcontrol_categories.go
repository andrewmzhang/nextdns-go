...
type ParentalControlCategoriesService interface {
	...
	List(
		context.Context,
		*ListParentalControlCategoriesRequest,
	) ([]*ParentalControlCategories, error)
...
func (s *parentalControlCategoriesService) Create(
	ctx context.Context,
	request *CreateParentalControlCategoriesRequest,
) error {
	path := fmt.Sprintf(
		"%s/%s",
		profileAPIPath(request.ProfileID),
		parentalControlCategoriesAPIPath,
	)
...
func (s *parentalControlCategoriesService) List(
	ctx context.Context,
	request *ListParentalControlCategoriesRequest,
) ([]*ParentalControlCategories, error) {
	path := fmt.Sprintf(
		"%s/%s",
		profileAPIPath(request.ProfileID),
	...
	if err != nil {
		return nil, fmt.Errorf(
			"error creating request to list the parental control categories: %w",
			err,
		)
	}
	...
	if err != nil {
		return nil, fmt.Errorf(
			"error making a request to list the parental control categories: %w",
			err,
		)
	}
...
func (s *parentalControlCategoriesService) Update(
	ctx context.Context,
	...
	path := fmt.Sprintf(
		"%s/%s",
		profileAPIPath(request.ProfileID),
		parentalControlCategoriesIDAPIPath(request.ID),
	)
	req, err := s.client.newRequest(http.MethodPatch, path, request.ParentalControlCategories)
	if err != nil {
		return fmt.Errorf(
			"error creating request to update the parental control categories: %w",
			err,
		)
	}
	...
	if err != nil {
		return fmt.Errorf(
			"error making a request to update the parental control categories: %w",
			err,
		)
	}
